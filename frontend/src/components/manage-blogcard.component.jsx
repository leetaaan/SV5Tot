    ...
    <div className="flex gap-2 max-lg:mb-6 max-lg:pb-6 border-grey border-b">
      {Object.keys(stats).map((key, i) => {
        return !key.includes("parent") ? (
          <div
            className={
              "flex flex-col items-center w-full h-full justify-center p-4 px-4 " +
...
export const ManagePublishedBlogCard = ({ blog }) => {
  ...
  let [showStat, serShowStat] = useState(false);
  ...
  return (
    <>
      <div className="flex gap-10 border-b mb-6 max-md:px-4 border-grey pb-6 items-center">
        <img
          src={banner}
          className="w-28 h-28 flex-none bg-grey object-cover max-md:hidden lg:hidden xl:block"
        />
        ...
        <div className="flex flex-col justify-between py-2 w-full min-w-[300px]">
          <div className="">
            ...
            <p className="line-clamp-1">Đăng vào {getDay(publishedAt)}</p>
          </div>
          ...
          <div className="flex gap-6 mt-3">
            ...
            <button
              onClick={() => serShowStat((preVal) => !preVal)}

...
const CommentCard = ({ index, leftVal, commentData }) => {
  ...
  const removeCommentsCards = (startingPoint, isDelete = false) => {
    ...
    if (commentData.childrenLevel === 0 && isDelete) {
      setTotalParentCommentsLoaded((perVal) => perVal - 1);
    }
    setBlog({
      ...blog,
      comments: { results: commentsArr },
  ...
  const handleReplyClick = () => {
    if (!access_token) {
      return toast.error("Đăng nhập để trả lời");
    }
      ...
      <div className="my-5 p-6 rounded-md border border-grey">
        ...
        <div className="flex gap-5 items-center mt-5">
          {commentData.isReplyLoaded ? (
            ...
            >
              <i className="fi fi-rs-comments-dots"></i>Ẩn trả lời
            </button>
          ) : (
            <button
              onClick={loadReplies}
              className="p-2 text-dark-grey px-3 hover:bg-grey/30 rounded-md flex items-center gap-2"
            >
              <i className="fi fi-rs-comments-dots"></i>
